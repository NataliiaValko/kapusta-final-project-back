...
userRouter.get(
  "/current",
  asyncWrapper(authenticateUser, userController.current)
);
userRouter.patch(
  "/avatar",
  asyncWrapper(authenticateUser, checkFilePresence, userController.changeAvatar)
);
userRouter.patch(
  "/update",
  asyncWrapper(authenticateUser, userUpdateValidation, userController.update)
);
