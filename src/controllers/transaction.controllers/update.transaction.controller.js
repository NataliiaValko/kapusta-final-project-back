...
const update = async (req, res, next) => {
  const { transaction, amountDifference } = await TransactionService.update(
    req.body,
    req.params
  );
  ...
  const balance = await UserService.changeBalance(
    data.transaction,
    false,
    amountDifference
  );

  res.status(200).json({ message: "success", data: { ...data, balance } });
};
